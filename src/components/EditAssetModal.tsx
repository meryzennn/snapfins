...
export default function EditAssetModal({
  ...
  const formattedInitialValue = useMemo(() => {
    ...
    const val =
      ...
      (initialData.valuation_mode === "manual"
        ? initialData.current_value
        : null);
    ...
    initialData.manual_value,
    initialData.current_value,
    initialData.valuation_mode,
    initialCurrency,
  ]);

  const [category, setCategory] = useState<AssetCategory>(
    (initialData.category as AssetCategory) || "Cash",
  );
  const [valuationMode, setValuationMode] = useState<ValuationMode>(
    (initialData.valuation_mode as ValuationMode) || "manual",
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!name.trim())
        throw new Error(
          lang === "id" ? "Nama aset wajib diisi" : "Asset name is required",
        );
      ...
      if (valuationMode === "manual") {
        ...
        if (!symbol.trim())
          throw new Error("Symbol required for market valuation");
  ...
  return createPortal(
    <div
      className="fixed inset-0 z-[100] flex items-start sm:items-center justify-center bg-black/60 backdrop-blur-sm p-4 pt-10 sm:pt-4 overflow-y-auto"
      onClick={onClose}
    >
      <div
        ...
        <div className="flex justify-between items-center mb-3 sm:mb-8 relative z-10 shrink-0">
          <div>
            <div className="flex items-center gap-2 sm:gap-3 mb-0.5 sm:mb-1">
              <span className="material-symbols-outlined text-primary bg-primary/10 p-1 sm:p-2 rounded-xl text-lg sm:text-2xl">
                edit
              </span>
            ...
            <p className="text-[10px] sm:text-sm text-on-surface-variant dark:text-gray-400 font-medium italic opacity-70">
              {step === 1
                ? lang === "id"
                  ? "Pilih tipe aset Anda"
                  : "Choose your asset type"
                : step === 2
                  ? lang === "id"
                    ? "Bagaimana kami akan menilai ini?"
                    : "How should we value this?"
                  : lang === "id"
                    ? "Ubah detail aset"
        ...
        <div className="flex-1 overflow-y-auto pr-1 -mr-1 scrollbar-thin scrollbar-thumb-outline-variant/30 scrollbar-track-transparent">
          ...
          {step === 1 && (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 animate-in slide-in-from-right-4 duration-500 overflow-y-auto pb-2 scrollbar-thin">
              {UI_CATEGORIES.map((cat) => (
                ...
                >
                  <div
                    className={`w-12 h-12 rounded-xl flex items-center justify-center mb-3 ${category === cat.name ? "bg-primary/10" : "bg-outline-variant/10"}`}
                  >
          ...
          {step === 2 && (
            <div className="flex flex-col gap-4 animate-in slide-in-from-right-4 duration-500">
              ...
              >
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${valuationMode === "manual" ? "bg-primary text-white" : "bg-outline-variant/20 text-on-surface"}`}
                >
                ...
                <div>
                  <span
                    className={`font-bold text-base block mb-1 ${valuationMode === "manual" ? "text-primary" : "text-on-surface"}`}
                  >
                    {lang === "id" ? "Input Manual" : "Manual Entry"}
                  </span>
                  <span className="text-sm font-medium text-on-surface-variant">
                    {lang === "id"
                      ? "Anda yang menentukan nilainya secara spesifik."
                      : "You explicitly define the total value."}
                  </span>
                </div>
              </button>
              ...
              {["Crypto", "Stock"].includes(category) ? (
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${valuationMode === "market" ? "bg-secondary text-white" : "bg-outline-variant/20 text-on-surface"}`}
                  >
                    <span className="material-symbols-outlined text-lg">
                      query_stats
                    </span>
                  ...
                  <div>
                    <span
                      className={`font-bold text-base block mb-1 flex items-center gap-2 ${valuationMode === "market" ? "text-secondary" : "text-on-surface"}`}
                    >
                      {lang === "id"
                    ...
                    <span className="text-sm font-medium text-on-surface-variant">
                      {lang === "id"
                        ? "Nilai dihitung otomatis via Harga Pasar × Kuantitas."
                        : "Value automatically derived from Live Market Price × Quantity."}
                    </span>
                  </div>
                </button>
              ) : (
                <button
                  disabled
                  className="flex items-start gap-4 p-5 rounded-2xl border-2 border-outline-variant/10 bg-surface-container-low/50 dark:bg-slate-800/30 opacity-60 cursor-not-allowed text-left relative overflow-hidden"
                >
                  <div className="absolute top-3 right-4">
                    <span className="text-[9px] uppercase tracking-widest font-black bg-outline-variant/20 text-on-surface-variant px-2 py-1 rounded-md">
                      Coming Soon
                    </span>
                  </div>
                  ...
                  <div className="pr-16">
                    ...
                    <span className="text-sm font-medium text-on-surface-variant">
                      {lang === "id"
                        ? "Akan terhubung langsung dengan mutasi."
                        : "Will derive balances automatically from accounts."}
                    </span>
                  </div>
                </button>
              )}
              ...
              <div className="space-y-4">
                ...
                {valuationMode === "market" ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1.5">
                      <label className="block text-[10px] font-black uppercase tracking-widest text-primary ml-1">
                        {category === "Gold" ? "SYMBOL" : "TICKER / SYMBOL"}{" "}
                        <span className="text-error">*</span>
                      </label>
                      {category === "Gold" ? (
                        ...
                        <input
                          ...
                          placeholder={
                            category === "Crypto" ? "BTC" : "NASDAQ:AAPL"
                          }
                          maxLength={20}
                        />
                      )}
