...
export default function AssetsPage() {
  ...
  const handleAddAsset = async (assetData: any) => {
    ...
    if (
      acquisition_mode === "via_transaction" &&
      source_asset_id &&
      purchase_amount
    ) {
      ...
      if (sourceAsset) {
        const newValue = Math.max(
          0,
          Number(sourceAsset.current_value) - Number(purchase_amount),
        );
        await supabase
          .from("assets")
  ...
  const handleEditAsset = async (assetData: any) => {
    ...
    const updatePayload = {
      ...assetData,
      last_valued_at:
        assetData.valuation_mode === "market"
          ? new Date().toISOString()
          : assetData.last_valued_at,
    };
  ...
  const normalizedAssets = assets.map((asset) => {
    ...
    const valInPrefCurrency = convert(
      rawVal,
      asset.currency as SupportedCurrency,
  ...
  const paginatedAssets = normalizedAssets.slice(
  ...
  const handleSelectAll = () => {
    if (
      selectedIds.length === normalizedAssets.length &&
    ...
    <>
      <main className="flex-grow max-w-7xl mx-auto w-full px-4 sm:px-6 md:px-8 py-6 md:py-10 space-y-8 md:space-y-10 pb-32 md:pb-12">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 md:gap-6">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            >
              <span className="material-symbols-outlined text-lg focus:outline-none">
                add
              </span>
          ...
          {[
            {
              ...
              gradient:
                "bg-gradient-to-br from-white/60 dark:from-slate-900/60 to-surface-container-low/40 dark:to-slate-800/40",
            ...
            },
            {
              ...
              icon: "water_drop",
              gradient:
                "bg-gradient-to-br from-white/60 dark:from-slate-900/60 to-secondary-container/10",
            ...
            <div
              key={idx}
              className={`glass-card p-6 rounded-2xl border border-white/40 dark:border-white/10 shadow-xl relative overflow-hidden group ${card.gradient}`}
            >
              <div
                className={`absolute -top-4 -right-4 w-24 h-24 rounded-full blur-2xl transition-colors ${card.iconBg}`}
              ></div>
              ...
              <div className="relative flex justify-between items-start mb-4 gap-4 mt-6">
                <div className="min-w-0 flex-grow">
                  ...
                  <div
                    className={`${card.val.length > 15 ? "text-xl" : "text-3xl"} ${card.textClass} font-headline tracking-tighter truncate`}
                    title={card.val}
                  >
                    {card.val}
                  </div>
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-3 pt-2">
              {allocEntries.map(([cat, val]) => (
                <div key={cat} className="flex items-center gap-2">
                  <span
                    className={`w-2.5 h-2.5 rounded-full ${getCategoryColor(cat)} shadow-sm`}
                  ></span>
                  <span className="font-bold text-[11px] text-on-surface uppercase tracking-wider">
                    {cat}
                  </span>
                  <span className="font-black text-[11px] text-on-surface-variant tabular-nums ml-1">
                    {(totalAssets > 0 ? (val / totalAssets) * 100 : 0).toFixed(
                      1,
                    )}
                    %
                  </span>
                ...
                <div className="flex flex-col gap-2 text-left max-w-xs w-full">
                  ...
                  <div className="flex items-start gap-2.5 text-[11px] text-on-surface-variant">
                    ...
                    <span>
                      {lang === "id" ? (
                        <>
                          <b>Tambah Saldo yang Dimiliki</b> — untuk akun bank
                          atau kas yang sudah ada.
                        </>
                      ) : (
                        <>
                          <b>Add Existing Balance</b> — for bank accounts or
                  ...
                  <div className="flex items-start gap-2.5 text-[11px] text-on-surface-variant">
                    ...
                    <span>
                      {lang === "id" ? (
                        <>
                          <b>Lalu catat pendapatan/pengeluaran</b> di Dashboard
                          menggunakan Manual Entry.
                        </>
                      ) : (
                        <>
                          <b>Then record income/expense</b> on the Dashboard
                          using Manual Entry.
                        </>
                      )}
                    </span>
                  ...
                  <div className="flex items-start gap-2.5 text-[11px] text-on-surface-variant">
                    <span className="material-symbols-outlined text-amber-400 text-[14px] mt-0.5 shrink-0">
                      looks_3
                    </span>
                    <span>
                      {lang === "id" ? (
                        <>
                          <b>Beli / Pindahkan ke Aset</b> untuk mencatat
                          pembelian saham/kripto dari kas Anda.
                        </>
                      ) : (
                        <>
                          <b>Buy / Move Into Asset</b> to record buying stocks
                          or crypto from your cash.
                        </>
                      )}
                    </span>
                  </div>
              ...
              <table className="w-full min-w-[650px] text-left excel-grid bg-surface-container-lowest dark:bg-slate-900/50 text-xs font-body tracking-tight">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-900 text-on-surface-variant text-[10px] uppercase tracking-widest font-black sticky top-0 z-20 border-b border-outline-variant/10 shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
                    ...
                    <th className="p-2.5 sm:p-4 text-right whitespace-nowrap">
                      {lang === "id" ? "Nilai" : "Value"}
                    </th>
                    <th className="p-2.5 sm:p-4 text-center whitespace-nowrap">
                ...
                <tbody className="text-on-surface text-sm font-semibold">
                  {paginatedAssets.map((asset) => (
                    <tr
                      key={asset.id}
                      className={`hover:bg-grid-row-hover dark:hover:bg-slate-800/50 transition-colors group ${selectedIds.includes(asset.id) ? "bg-primary/[0.08]" : ""}`}
                    >
                      ...
                      <td className="p-2.5 sm:p-4">
                        <div className="flex flex-col">
                          <span className="text-on-surface font-bold text-[13px] sm:text-[14px] truncate max-w-[100px] sm:max-w-xs block">
                            {asset.name}
                          </span>
                          <div className="flex items-center gap-1.5 mt-0.5">
                            {asset.symbol && (
                              <span className="text-[9px] sm:text-[10px] font-black tracking-widest text-primary uppercase">
                                {asset.symbol}
                              </span>
                            )}
                            {asset.quantity && Number(asset.quantity) > 0 && (
                              <>
                                ...
                                <span className="text-[9px] sm:text-[10px] font-black tracking-widest text-on-surface-variant/70 uppercase">
                        ...
                        <div className="flex flex-col items-end">
                          ...
                          <div className="flex items-center gap-1 mt-0.5">
                            {asset.valuation_mode === "manual" ? (
                              <span className="text-[8px] sm:text-[9px] uppercase tracking-widest text-on-surface-variant font-bold">
                                MANUAL
                              </span>
                              ...
                              <div className="flex flex-col items-end">
                                <div className="flex items-center gap-1">
                                  ...
                                  <span className="text-[8px] sm:text-[9px] uppercase tracking-widest text-secondary font-black">
                                    MARKET
                                  </span>
                                </div>
                                <span className="text-[7px] sm:text-[8px] uppercase tracking-widest text-on-surface-variant font-bold opacity-70 mt-0.5">
                                  {asset.exchange === "COINGECKO" ||
                                  asset.exchange === "CRYPTO"
                                    ? "COINGECKO"
                                    : asset.exchange && asset.exchange !== "US"
                                      ? asset.exchange
                                      : "YAHOO FIN"}
                                </span>
                              </div>
                      ...
                      <td className="p-2.5 sm:p-4 text-center text-on-surface-variant font-bold text-[10px] sm:text-[11px] uppercase tracking-widest">
                        {new Date(asset.last_valued_at).toLocaleDateString(
                          lang === "id" ? "id-ID" : "en-US",
      ...
      {showAddAssetModal && (
        <AddAssetModal
          ...
          cashAssets={assets.filter((a) =>
            ["Cash", "Bank", "E-wallet"].includes(a.category),
          )}
        />
      )}
