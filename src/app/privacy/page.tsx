    ...
    <div className="min-h-screen bg-surface-container-lowest dark:bg-slate-950 text-on-surface font-body font-medium selection:bg-primary/20 transition-colors duration-500">
      ...
      <main className="pt-20 pb-24 px-6 max-w-4xl mx-auto text-on-surface-variant leading-relaxed space-y-10">
        ...
        <section className="space-y-4">
          <h2 className="text-2xl font-bold font-headline text-on-surface dark:text-white">{t('privacyIntroTitle')}</h2>
          <p className="dark:text-slate-400">{t('privacyIntroText')}</p>
        </section>
 
        <section className="space-y-4">
          <h2 className="text-2xl font-bold font-headline text-on-surface dark:text-white">{t('privacyDataTitle')}</h2>
          <ul className="list-disc leading-loose ml-6 space-y-2 dark:text-slate-400">
            <li><strong className="text-on-surface dark:text-white">{t('privacyDataList1').split(':')[0]}:</strong>{t('privacyDataList1').split(':')[1]}</li>
            <li><strong className="text-on-surface dark:text-white">{t('privacyDataList2').split(':')[0]}:</strong>{t('privacyDataList2').split(':')[1]}</li>
            <li><strong className="text-on-surface dark:text-white">{t('privacyDataList3').split(':')[0]}:</strong>{t('privacyDataList3').split(':')[1]}</li>
          </ul>
        </section>
 
        <section className="space-y-4">
          <h2 className="text-2xl font-bold font-headline text-on-surface dark:text-white">{t('privacyAiTitle')}</h2>
        ...
        <section className="space-y-4 mt-10">
          <p className="dark:text-slate-400">
            {t('privacyContact')}{' '}
            <a href="mailto:privacy@snapfins.0x5zen.dev" className="text-primary font-bold hover:underline">
              privacy@snapfins.0x5zen.dev
            </a>.
          </p>
        </section>
      </main>
